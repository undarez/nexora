...
async function stripeFetch(path: string, form: URLSearchParams, idempotencyKey?: string) {
  ...
  if (!response.ok) {
    const message = body?.error?.message ?? (typeof body === "string" ? body : "Erreur Stripe.");
    throw new Error(`Stripe API ${response.status}: ${message}`);
  }
  return body;
}
...
export async function createStripePaymentIntent(input: StripePaymentIntentInput): Promise<StripePaymentIntent> {
  ...
  const form = encodeForm({
    amount: String(input.amount), currency, customer: input.customerId, description: input.description,
...
export function parseStripeWebhookEvent(rawBody: string): StripeWebhookEvent {
  ...
  try { event = JSON.parse(rawBody) as StripeWebhookEvent; } catch { throw new Error("Payload webhook Stripe invalide."); }
  if (!event || typeof event.id !== "string" || typeof event.type !== "string") throw new Error("Événement Stripe invalide.");
  return event;
}
...
export async function createStripeCustomer(input: { userId: string; email?: string | null; name?: string | null; idempotencyKey: string }): Promise<StripeCustomerRecord> {
  if (!/^[A-Za-z0-9._:-]{8,255}$/.test(input.idempotencyKey)) throw new Error("Clé d'idempotence Stripe invalide.");
  const metadata = { nexora_user_id: input.userId };
  return stripeFetch("/customers", encodeForm({ email: input.email ?? undefined, name: input.name ?? undefined, ...Object.fromEntries(Object.entries(metadata).map(([k,v]) => [`metadata[${k}]`,v])) }), input.idempotencyKey);
}

export async function createStripeSetupIntent(input: { customerId: string; userId: string; paymentMethodTypes?: string[]; idempotencyKey: string }): Promise<StripeSetupIntent> {
  const types = input.paymentMethodTypes?.length ? input.paymentMethodTypes : ["sepa_debit"];
  const form = encodeForm({ customer: input.customerId, usage: "off_session", ...Object.fromEntries(types.map((type, i) => [`payment_method_types[${i}]`, type])), "metadata[nexora_user_id]": input.userId });
  return stripeFetch("/setup_intents", form, input.idempotencyKey);
}

export async function retrieveStripePaymentMethod(paymentMethodId: string) {
  if (!/^pm_[A-Za-z0-9]+$/.test(paymentMethodId)) throw new Error("PaymentMethod Stripe invalide.");
  const response = await fetch(`${STRIPE_API}/payment_methods/${encodeURIComponent(paymentMethodId)}`, { headers: { Authorization: `Bearer ${secretKey()}` }, cache: "no-store" });
  const text = await response.text(); let body:any=null; try { body=text?JSON.parse(text):null } catch { body=text }
  if (!response.ok) throw new Error(`Stripe API ${response.status}: ${body?.error?.message ?? "Erreur Stripe."}`);
  return body;
}

export async function createStripeRefund(input: { paymentIntentId: string; userId: string; amount?: number; reason?: string; idempotencyKey: string }): Promise<StripeRefund> {
  if (!/^pi_[A-Za-z0-9]+$/.test(input.paymentIntentId)) throw new Error("PaymentIntent Stripe invalide.");
  const form = encodeForm({ payment_intent: input.paymentIntentId, amount: input.amount !== undefined ? String(input.amount) : undefined, reason: input.reason, "metadata[nexora_user_id]": input.userId });
  return stripeFetch("/refunds", form, input.idempotencyKey);
}
