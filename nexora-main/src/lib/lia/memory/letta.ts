...
export async function ensureLettaAgent(supabase: SupabaseClient, userId: string) {
  ...
  // Use a service-role client only inside this server module.
...
/**
 * Recall is deliberately narrow: it asks Letta for memory, but callers must
 * treat the result as contextual evidence, never as financial truth.
 */
...
/**
 * Only explicit user memory intent is persisted here. Ordinary financial
 * conversations are not copied wholesale into the external memory provider.
 */
