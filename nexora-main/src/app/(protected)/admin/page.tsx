...
export default async function AdminPage() {
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-6 py-8">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Link href="/admin/open-banking"><Card className="h-full transition hover:-translate-y-0.5 hover:shadow-lg"><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" />Open Banking</CardTitle></CardHeader><CardContent className="text-sm text-muted-foreground">Connexions, comptes, synchronisations, statuts et échéances de reconnexion 90 jours.</CardContent></Card></Link><Link href="/admin/analytics"><Card className="h-full transition hover:-translate-y-0.5 hover:shadow-lg"><CardHeader><CardTitle className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-primary" />Analytics produit</CardTitle></CardHeader><CardContent className="text-sm text-muted-foreground">Trafic, utilisateurs actifs, sessions et modules les plus consultés. Vue strictement réservée à l’administration.</CardContent></Card></Link><Link href="/admin/lia-learning"><Card className="h-full transition hover:-translate-y-0.5 hover:shadow-lg"><CardHeader><CardTitle className="flex items-center gap-2"><Bot className="h-5 w-5 text-primary" />Learning Review Board</CardTitle></CardHeader><CardContent className="text-sm text-muted-foreground">Revue humaine des améliorations candidates de LIA, replay et non-régression.</CardContent></Card></Link><Link href="/admin/lia-crons"><Card><CardHeader><CardTitle className="flex items-center gap-2"><CalendarClock className="h-5 w-5 text-primary" />Cron autonomes LIA</CardTitle></CardHeader><CardContent className="text-sm text-muted-foreground">Voir tous les Cron créés par LIA, leur justification, leur historique et intervenir immédiatement si nécessaire.</CardContent></Card></Link><Card><CardHeader><CardTitle className="flex items-center gap-2"><Settings className="h-5 w-5 text-primary" />Système</CardTitle></CardHeader><CardContent className="text-sm text-muted-foreground">Les contrôles techniques restent réservés à l'administration et ne sont jamais exposés à l'utilisateur final.</CardContent></Card>
    </div>
  </main>;
}
