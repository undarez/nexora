...
export async function POST(request:Request){
 ...
 try{
  ...
  if(!domainPolicy.allowed)return NextResponse.json({error:"research_domain_not_allowed",reason:domainPolicy.reason,domain:requestedHost},{status:403});
  if(!domainPolicy.entry)return NextResponse.json({error:"research_domain_not_registered",domain:requestedHost},{status:403});
  const entry=domainPolicy.entry;
  const result=await acquireSource({url:body.url,maxBytes:body.maxBytes,timeoutMs:body.timeoutMs,maxRedirects:body.maxRedirects,allowedHosts:[entry.domain]});
  ...
  return NextResponse.json({...result,trust:{sourceClass:entry.sourceClass,trustScore:entry.trustScore,organization:entry.organization,freshnessDays:entry.freshnessDays}});
