...
export async function ensureLiaAutonomousCron(supabase: SupabaseClient, userId: string) {
  ...
  const allJobs = jobs ?? []; const managedJobs = allJobs.filter(job => typeof job.payload?.action === "string" && job.payload.action === "proactive_financial_watch"); const learningJobs = allJobs.filter(job => typeof job.payload?.action === "string" && job.payload.action === "autonomous_learning"); const goalJobs = allJobs.filter(job => typeof job.payload?.action === "string" && job.payload.action === "autonomous_goal_watch"); const continuousJobs = allJobs.filter(job => typeof job.payload?.action === "string" && job.payload.action === "continuous_operations");
  ...
  let continuousOperationsScheduled = Boolean(continuousJobs.find(job => ["ready", "running"].includes(job.status))); let continuousOperationsCreated = false;
  if (hasFinancialContext && !continuousOperationsScheduled) {
    const schedule = "0 20 * * *";
    const { data: continuousJob, error: continuousError } = await admin.from("lia_runtime_jobs").insert({ user_id: userId, runtime_type: "cron", name: "LIA · Opérations continues", description: "Wake cycle quotidien borné : récupération, objectifs, apprentissage ou observation selon l'état réel.", schedule, status: "ready", next_run_at: nextCronRun(schedule, new Date(), "Europe/Paris")?.toISOString() ?? null, payload: { action: "continuous_operations", execution_mode: "agent", managed_by: "lia", adaptive: true, financial_writes_allowed: false }, requires_policy_gate: true, requires_human_approval: false, timezone: "Europe/Paris", execution_mode: "agent" }).select("id,schedule").single();
    if (!continuousError && continuousJob) { continuousOperationsScheduled = true; continuousOperationsCreated = true; await admin.from("lia_runtime_events").insert({ user_id: userId, runtime_job_id: continuousJob.id, runtime_type: "cron", event: "cron.scheduled", status: "completed", payload: { schedule, purpose: "continuous_operations", governed: true } }); }
  }
