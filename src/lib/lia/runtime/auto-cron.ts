...
function adminClient() {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL; const secret = process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
...
export async function ensureLiaAutonomousCron(supabase: SupabaseClient, userId: string) {
  ...
  const { data: jobs, error: jobsError } = await admin.from("lia_runtime_jobs").select("id,name,status,schedule,payload,next_run_at,updated_at,admin_disabled").eq("user_id", userId).eq("runtime_type", "cron").limit(50);
  ...
  const goalJobs = allJobs.filter(job => typeof job.payload?.action === "string" && job.payload.action === "autonomous_goal_watch");
  ...
  const { count: activeAutonomousGoals } = await supabase.from("agent_loop_runs").select("id", { count: "exact", head: true }).eq("user_id", userId).in("status", ["running", "blocked", "needs_human"]).not("context->goal_lifecycle->>state", "in", "(completed,failed)");
  ...
  if (hasFinancialContext) {
    ...
    if (existing && existing.schedule !== schedule) {
      await admin.from("lia_runtime_jobs").update({ status: "paused", last_status: "replaced_by_usage_policy", updated_at: new Date().toISOString() }).eq("id", existing.id); surveillanceJobId = null;
    } else if (existing && !existing.next_run_at) {
      const nextRun = nextCronRun(schedule, new Date(), "Europe/Paris")?.toISOString() ?? null; if (nextRun) await admin.from("lia_runtime_jobs").update({ next_run_at: nextRun, updated_at: new Date().toISOString() }).eq("id", existing.id);
    }
    if (!surveillanceJobId) {
      ...
      surveillanceJobId = job.id; surveillanceCreated = true;
  ...
  if (learningExisting) {
    ...
    if (!learningExisting.next_run_at) { const nextRun = nextCronRun(learningSchedule, new Date(), "Europe/Paris")?.toISOString() ?? null; if (nextRun) await admin.from("lia_runtime_jobs").update({ next_run_at: nextRun, updated_at: new Date().toISOString() }).eq("id", learningExisting.id); }
  } else {
    const learningSchedule = "30 8 * * 1";
    const { data: learningJob, error: learningError } = await admin.from("lia_runtime_jobs").insert({ user_id: userId, runtime_type: "cron", name: "LIA · Apprentissage hebdomadaire", description: "Boucle autonome de recherche, vérification et apprentissage borné de LIA.", schedule: learningSchedule, status: "ready", next_run_at: nextCronRun(learningSchedule, new Date(), "Europe/Paris")?.toISOString() ?? null, payload: { action: "autonomous_learning", execution_mode: "agent", managed_by: "lia", adaptive: true, financial_writes_allowed: false, trusted_domains_only: true }, requires_policy_gate: true, requires_human_approval: false, timezone: "Europe/Paris", execution_mode: "agent" }).select("id,schedule").single();
    if (!learningError && learningJob) {
      learningScheduled = true; learningCreated = true;
      await admin.from("lia_runtime_events").insert({ user_id: userId, runtime_job_id: learningJob.id, runtime_type: "cron", event: "cron.scheduled", status: "completed", payload: { schedule: learningSchedule, managed_by: "lia", purpose: "autonomous_learning", trusted_domains_only: true } });
    }
  }

  let goalWatchScheduled = Boolean(goalJobs.find(job => ["ready", "running"].includes(job.status))); let goalWatchCreated = false;
  if (Number(activeAutonomousGoals ?? 0) > 0) {
    const existing = goalJobs.find(job => ["ready", "running"].includes(job.status));
    if (!existing) {
      const schedule = "15 9 * * *";
      const { data: goalJob, error: goalError } = await admin.from("lia_runtime_jobs").insert({ user_id: userId, runtime_type: "cron", name: "LIA · Continuation des objectifs", description: "Réévaluation bornée des objectifs autonomes actifs avec mémoire et vérification persistantes.", schedule, status: "ready", next_run_at: nextCronRun(schedule, new Date(), "Europe/Paris")?.toISOString() ?? null, payload: { action: "autonomous_goal_watch", execution_mode: "agent", managed_by: "lia", financial_writes_allowed: false, max_steps: 8 }, requires_policy_gate: true, requires_human_approval: true, timezone: "Europe/Paris", execution_mode: "agent" }).select("id,schedule").single();
      if (!goalError && goalJob) {
        goalWatchScheduled = true; goalWatchCreated = true;
        await admin.from("lia_runtime_events").insert({ user_id: userId, runtime_job_id: goalJob.id, runtime_type: "cron", event: "cron.scheduled", status: "completed", payload: { schedule, purpose: "autonomous_goal_watch", active_goals: Number(activeAutonomousGoals ?? 0) } });
      }
    }
  } else {
    const staleJobs = goalJobs.filter(job => ["ready", "running"].includes(job.status));
    for (const job of staleJobs) await admin.from("lia_runtime_jobs").update({ status: "paused", last_status: "no_active_autonomous_goals", updated_at: new Date().toISOString() }).eq("id", job.id);
    goalWatchScheduled = false;
  }

  return { status: surveillanceCreated || learningCreated || goalWatchCreated ? "created" as const : "unchanged" as const, jobId: surveillanceJobId, schedule: hasFinancialContext ? (activity >= 20 ? "0 8 * * *" : "0 8 * * 1") : null, learningScheduled, learningCreated, surveillanceCreated, goalWatchScheduled, goalWatchCreated, activeAutonomousGoals: Number(activeAutonomousGoals ?? 0) };
}
