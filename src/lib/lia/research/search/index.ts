...
class TavilyProvider implements SearchProvider {
 ...
 async search(query:string,limit:number){const key=process.env.TAVILY_API_KEY;if(!key)throw new Error("search_provider_not_configured");
  ...
  const {data:budget,error:budgetError}=await admin.rpc("lia_consume_research_provider_budget",{p_provider:"tavily",p_month_start:monthStart.toISOString().slice(0,10),p_credits:1,p_monthly_limit:Number.isFinite(monthlyLimit)&&monthlyLimit>0?Math.floor(monthlyLimit):1000,p_guard_percent:Number.isFinite(guardPercent)&&guardPercent>0?Math.min(100,Math.floor(guardPercent)):90});
...
function configuredProviders(disabledProviders: string[] = []):SearchProvider[]{
 ...
 if((selected==="tavily"||selected==="auto")&&!disabled.has("tavily"))providers.push(new TavilyProvider());
...
export async function discoverTrustedSources(query:string,limit=5,disabledProviders:string[]=[]){const providers=configuredProviders(disabledProviders);if(!providers.length)return {provider:null,providers:[],results:[],status:"not_configured" as const};const errors:string[]=[];const raw:SearchResult[]=[];const settled=await Promise.allSettled(providers.map(p=>p.search(query,clamp(limit*3,3,20))));settled.forEach((result,index)=>{if(result.status==="fulfilled")raw.push(...result.value);else errors.push(`${providers[index].name}:${result.reason instanceof Error?result.reason.message:"search_failed"}`);});const results=rankSearchResults(raw,limit);return {provider:results[0]?.provider||providers[0]?.name||null,providers:providers.map(p=>p.name),results,status:results.length?"ok" as const:errors.length?"search_failed" as const:"no_results" as const,errors:errors.length?errors:undefined};}
