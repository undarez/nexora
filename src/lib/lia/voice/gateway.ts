...
async function hume(request:VoiceRequest){
  const key=process.env.HUME_API_KEY;if(!key)throw new Error("HUME_API_KEY n'est pas configurée.");
  const utterance:any={text:cleanText(request.text)};const voiceId=request.voiceId||process.env.HUME_VOICE_ID;const voiceName=request.voiceName||process.env.HUME_VOICE_NAME;
  if(voiceId)utterance.voice={id:voiceId};else if(voiceName)utterance.voice={name:voiceName,provider:"HUME_AI"};
  ...
  if(!r.ok)throw new Error(`Hume TTS HTTP ${r.status}`);return Buffer.from(await r.arrayBuffer());
}
async function elevenlabs(request:VoiceRequest){
  const key=process.env.ELEVENLABS_API_KEY;const voice=request.voiceId||process.env.ELEVENLABS_VOICE_ID;
  ...
  if(!r.ok)throw new Error(`ElevenLabs TTS HTTP ${r.status}`);return Buffer.from(await r.arrayBuffer());
}
export async function synthesizeVoice(request:VoiceRequest){const provider=request.provider||((process.env.NEXORA_VOICE_PROVIDER as VoiceProvider)||"hume");if(provider==="elevenlabs")return elevenlabs(request);return hume(request);}
