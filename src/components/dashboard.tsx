...
export function Dashboard(){
 ...
 const decision=supervise({projectedCashflow:calculations.projected,essentialsCovered:calculations.projected>=0,proposedInvestment:0,liquidReserve:realBalance,targetReserve:currentScenario.safety_reserve});
 ...
 const chart = [
   ["Départ",currentScenario.starting_balance],
   ["+ Revenus",currentScenario.starting_balance+currentScenario.income],
   ["- Fixes",currentScenario.starting_balance+currentScenario.income-fixedTotal],
   ["- Variables",calculations.projected+currentScenario.extra_expense],
   ["Fin de mois",calculations.projected]
 ] as [string,number][];
 ...
 const chartPoints = chart.map(([,v], i) => `${12 + i * 21},${88 - ((v - Math.min(...chart.map(([,x])=>x))) / Math.max(Math.max(...chart.map(([,x])=>x)) - Math.min(...chart.map(([,x])=>x)),1)) * 64}`).join(" ");
 ...
 return <main className="dashboard-page nexora-reference-dashboard mx-auto max-w-7xl space-y-5 px-4 pb-24 pt-5 sm:px-6 sm:pb-10 sm:pt-7">
   <div className="dashboard-page-heading">
     <div><p className="nexora-kicker">Vue d’ensemble</p><h1>Bonjour{displayName ? `, ${displayName}` : ""} 👋</h1><p>Voici un aperçu de votre situation financière aujourd’hui.</p></div>
     <div className="dashboard-date-pill"><CalendarDays className="h-4 w-4"/><span className="capitalize">{monthLabel(month)}</span><ChevronRight className="h-4 w-4"/></div>
   </div>

   <section className="nexora-dashboard-topgrid">
     <Card className="nexora-networth-card">
       <CardContent className="p-0">
         <div className="nexora-networth-inner">
           <div className="nexora-networth-head"><div><p>Valeur totale de vos liquidités</p><span className="nexora-eye"><CircleDollarSign className="h-4 w-4"/> Situation actuelle</span></div><span className="nexora-growth-pill">↗ {realIncome > 0 ? "+" : ""}{realIncome ? Math.round((realIncome / Math.max(realBalance,1))*100) : 0}%</span></div>
           <div className="nexora-networth-value">{money(realBalance)} €</div>
           <div className="nexora-networth-change"><span>{netMonth >= 0 ? "↗" : "↘"} {moneyWithSign(netMonth)}</span><small>ce mois-ci</small></div>
           <div className="nexora-line-chart" aria-label="Évolution de la situation financière"><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img"><defs><linearGradient id="nexoraArea" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="currentColor" stopOpacity=".28"/><stop offset="100%" stopColor="currentColor" stopOpacity="0"/></linearGradient></defs><polygon points={`12,100 ${chartPoints} 96,100`} fill="url(#nexoraArea)"/><polyline points={chartPoints} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg></div>
           <div className="nexora-periods"><span>1J</span><span>1S</span><b>1M</b><span>3M</span><span>1A</span><span>TOUT</span></div>
         </div>
       </CardContent>
     </Card>

     <Card className="nexora-goals-card"><CardHeader><div className="flex items-center justify-between"><CardTitle>Progression vers vos objectifs</CardTitle><Link href="/pilotage" className="nexora-card-link">Voir tout</Link></div></CardHeader><CardContent>
       {primaryGoal ? <div className="nexora-goal-main"><div className="nexora-donut" style={{"--progress":`${goalProgress}%`} as any}><strong>{Math.round(goalProgress)}%</strong></div><div className="min-w-0"><div className="nexora-goal-icon"><Target className="h-5 w-5"/></div><p className="font-bold">{primaryGoal.name}</p><p className="text-sm text-muted-foreground">{money(primaryGoal.current_amount)} € / {money(primaryGoal.target_amount)} €</p><small>{primaryGoal.target_date ? `Échéance ${new Intl.DateTimeFormat("fr-FR",{month:"long",year:"numeric"}).format(new Date(`${primaryGoal.target_date}T12:00:00`))}` : "Objectif en cours"}</small></div></div> : <div className="nexora-empty-state"><Target className="h-7 w-7"/><p>Aucun objectif configuré</p><Link href="/pilotage">Créer un objectif</Link></div>}
       {goals.slice(1,3).map((goal)=><div key={goal.name} className="nexora-goal-row"><span className="nexora-goal-mini"><Target className="h-4 w-4"/></span><div className="min-w-0 flex-1"><strong className="block truncate">{goal.name}</strong><small>{money(goal.current_amount)} € / {money(goal.target_amount)} €</small></div><ChevronRight className="h-4 w-4 text-muted-foreground"/></div>)}
     </CardContent></Card>
   </section>

   <section className="nexora-kpi-grid">
     <div className="nexora-kpi"><span className="nexora-kpi-icon blue"><Landmark/></span><div><small>Solde disponible</small><strong>{money(realBalance)} €</strong><em>↗ Situation actuelle</em></div></div>
     <div className="nexora-kpi"><span className="nexora-kpi-icon cyan"><ArrowDownRight/></span><div><small>Total des revenus</small><strong>{money(realIncome)} €</strong><em>↗ Ce mois-ci</em></div></div>
     <div className="nexora-kpi"><span className="nexora-kpi-icon red"><ArrowUpRight/></span><div><small>Total des dépenses</small><strong>{money(financialContext?.cashflow.expenses ?? 0)} €</strong><em className="negative">{financialContext?.cashflow.expenses ? "↗ Flux sortants" : "— Aucun flux"}</em></div></div>
     <div className="nexora-kpi"><span className="nexora-kpi-icon green"><PiggyBank/></span><div><small>Épargne ce mois-ci</small><strong>{money(savings)} €</strong><em>↗ Marge disponible</em></div></div>
     <div className="nexora-kpi"><span className="nexora-kpi-icon purple"><TrendingUp/></span><div><small>Projection fin de mois</small><strong>{moneyWithSign(calculations.projected)}</strong><em className={risk === "high" ? "negative" : ""}>{risk === "high" ? "⚠ Vigilance" : "✓ Projection maîtrisée"}</em></div></div>
   </section>

   <section className="nexora-dashboard-grid-2">
     <Card><CardHeader><div className="flex items-center justify-between"><div><CardTitle>Évolution de votre situation</CardTitle><p className="mt-1 text-sm text-muted-foreground">Revenus, charges et projection du mois.</p></div><span className="nexora-select-pill">1 mois <ChevronRight className="h-3.5 w-3.5"/></span></div></CardHeader><CardContent><div className="nexora-performance-value">{moneyWithSign(netMonth)} <span>{netMonth >= 0 ? "positif" : "à surveiller"}</span></div><div className="nexora-performance-chart"><svg viewBox="0 0 100 100" preserveAspectRatio="none"><polyline points={chartPoints} fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/><line x1="0" x2="100" y1="88" y2="88" stroke="currentColor" strokeOpacity=".12"/></svg></div><div className="nexora-chart-labels"><span>Départ</span><span>Revenus</span><span>Fixes</span><span>Variables</span><span>Projection</span></div></CardContent></Card>
     <Card><CardHeader><div className="flex items-center justify-between"><CardTitle>Répartition de vos comptes</CardTitle><Link href="/banque" className="nexora-card-link">Voir le détail</Link></div></CardHeader><CardContent><div className="nexora-allocation"><div className="nexora-allocation-donut" style={{"--connected":`${connectedPct}%`} as any}><strong>{money(liquidTotal)} €</strong><small>Valeur totale</small></div><div className="nexora-allocation-legend"><div><i className="dot purple"/><span>Comptes connectés</span><b>{connectedPct}%</b></div><div><i className="dot blue"/><span>Comptes manuels</span><b>{manualPct}%</b></div></div></div></CardContent></Card>
   </section>

   <section className="nexora-dashboard-grid-3">
     <Card><CardHeader><div className="flex items-center justify-between"><CardTitle>À surveiller</CardTitle><Link href="/pilotage" className="nexora-card-link">Voir tout</Link></div></CardHeader><CardContent className="space-y-2.5">{anomalies.slice(0,4).map((item)=><div key={item.id} className="nexora-insight-row"><span className="nexora-insight-icon warning"><AlertTriangle/></span><div><strong>{item.label}</strong><p>{money(item.amount)} € · {item.category}</p></div></div>)}{anomalies.length===0&&<div className="nexora-insight-row"><span className="nexora-insight-icon success"><ShieldCheck/></span><div><strong>Aucune anomalie détectée</strong><p>Les flux observés restent cohérents.</p></div></div>}</CardContent></Card>
     <Card><CardHeader><div className="flex items-center justify-between"><CardTitle>Transactions récentes</CardTitle><Link href="/transactions" className="nexora-card-link">Voir tout</Link></div></CardHeader><CardContent className="space-y-1">{recentTransactions.slice(0,5).map(tx=><div key={tx.id} className="nexora-transaction-row"><span className={`nexora-tx-icon ${tx.amount >= 0 ? "income" : "expense"}`}>{tx.amount >= 0 ? <ArrowDownRight/> : <ArrowUpRight/>}</span><div className="min-w-0 flex-1"><strong className="block truncate">{tx.label}</strong><small>{tx.date ? new Intl.DateTimeFormat("fr-FR",{day:"numeric",month:"short"}).format(new Date(`${tx.date}T12:00:00`)) : ""} · {tx.category}</small></div><b className={tx.amount >= 0 ? "positive" : "negative"}>{moneyWithSign(tx.amount)}</b></div>)}{recentTransactions.length===0&&<p className="py-5 text-sm text-muted-foreground">Aucune transaction récente.</p>}</CardContent></Card>
     <Card><CardHeader><div className="flex items-center justify-between"><CardTitle>Analyses & conseils</CardTitle><Link href="/lia" className="nexora-card-link">Voir tout</Link></div></CardHeader><CardContent className="space-y-2.5"><div className="nexora-insight-row"><span className="nexora-insight-icon success"><TrendingUp/></span><div><strong>{firstInsight}</strong><p>{calculations.projected >= currentScenario.safety_reserve ? "Bonne marge par rapport à votre réserve." : "LIA peut vous aider à ajuster le budget."}</p></div></div><div className="nexora-insight-row"><span className="nexora-insight-icon purple"><Sparkles/></span><div><strong>{recurring.length ? `${recurring.length} flux récurrent(s) identifié(s)` : "Analysez vos flux récurrents"}</strong><p>Utilisez LIA pour détecter les économies possibles.</p></div></div><div className="nexora-insight-row"><span className="nexora-insight-icon blue"><ShieldCheck/></span><div><strong>Votre budget est surveillé</strong><p>{fixed.length} engagement(s) fixe(s) pris en compte.</p></div></div></CardContent></Card>
   </section>
