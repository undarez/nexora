...
export function MonthlyTransactionsBridge() {
  ...
  useEffect(() => {
    ...
    element.className = "nexora-monthly-transactions-slot mx-auto w-full max-w-7xl px-4 sm:px-6";
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const load = async () => {
      if (pathname === "/transactions") await fetch("/api/banking/classify", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" }).catch(() => null);
      ...
      const rows: MonthlyAnalyticsTransaction[] = [];
      ...
      for (const tx of manual.data ?? []) {
        const category = Array.isArray(tx.categories) ? tx.categories[0] : tx.categories;
        rows.push({ id: `manual:${tx.id}`, amount: Number(tx.amount), occurred_at: String(tx.occurred_at), label: String(tx.label || "Transaction"), category: category?.name ? String(category.name) : null });
      }
  ...
  useEffect(() => {
    ...
    const autoCategorize = async () => {
      ...
      const [categoriesResult, transactionsResult] = await Promise.all([
        ...
        supabase.from("transactions").select("id,label,amount,category_id").eq("user_id", user.id).gte("occurred_at", start).lt("occurred_at", end).is("category_id", null).limit(500),
      ]);
      if (cancelled || transactionsResult.error) return;
      ...
      for (const tx of transactionsResult.data ?? []) {
        ...
        const existing = categories.find((category: any) => classifyTransaction({ label: category.name, amount: Number(tx.amount) }).key === definition.key);
        let categoryId = existing?.id ?? null;
        if (!categoryId) {
          const created = await supabase.from("categories").insert({ user_id: user.id, name: definition.label, kind: Number(tx.amount) < 0 ? "expense" : "income" }).select("id,name,kind").single();
          if (!created.error) { categoryId = created.data.id; categories.push(created.data as any); }
        }
